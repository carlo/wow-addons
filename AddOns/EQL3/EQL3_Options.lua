...
function EQL_Options_SetStates()
		...
		if(QuestlogOptions[EQL3_Player].ActiveTracker == 1) then
			EQL3_OptionsFrame_Checkbox_ActiveTracker:SetChecked(1);
		else
			EQL3_OptionsFrame_Checkbox_ActiveTracker:SetChecked(0);
		end
		
		if(QuestlogOptions[EQL3_Player].ShowTrackerHeader == 1) then
			EQL3_OptionsFrame_Checkbox_ShowTrackerHeader:SetChecked(1);
		else
			EQL3_OptionsFrame_Checkbox_ShowTrackerHeader:SetChecked(0);
		end
end
...
function EQL3_UpdateDB()
	...
	local index;
	for index in pairs(QuestlogOptions[EQL3_Player].OrganizerSettings) do
...
--		questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete = old_GetQuestLogTitle(i);
		questLogTitleText, level, questTag, _, isHeader, isCollapsed, isComplete = old_GetQuestLogTitle(i);
		if(isHeader) then
...
--					temp2, _, _, isHeader = GetQuestLogTitle(j);
					temp2, _, _, _, isHeader = GetQuestLogTitle(j);
...
function GetQuestIndexForWatch(watchID)
	...
	if(EQL3_Temp.savedQuestIDMap) then
		...
		local questID;
		for questID in pairs(EQL3_Temp.savedQuestIDMap) do
...
function EQL3_RealmDropDown_Initialize()
	...
	if ( UIDROPDOWNMENU_MENU_LEVEL == 1 ) then
		...
		for i in pairs(QuestlogOptions) do
			for w in string.gmatch(i, "%-([^%-]+)") do
...
function EQL3_LoadCharacterSettings()
	...
	--load the settings...
		...
		QuestlogOptions[EQL3_Player].ActiveTracker = QuestlogOptions[this.value].ActiveTracker;
...
-- new to 3.6.2
function EQL3_Toggle_ActiveTracker()
	if (this:GetChecked() == 1) then
		QuestlogOptions[EQL3_Player].ActiveTracker = 1;
	else
		QuestlogOptions[EQL3_Player].ActiveTracker = 0;
	end
end

function EQL3_Toggle_ShowTrackerHeader()
	if (this:GetChecked() == 1) then
		QuestlogOptions[EQL3_Player].ShowTrackerHeader = 1;
	else
