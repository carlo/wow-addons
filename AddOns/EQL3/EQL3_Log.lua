...
function EQL_QuestLog_OnEvent(event)
	if (event == "VARIABLES_LOADED") then
		...
		-- 3.6.2
		if(QuestlogOptions[EQL3_Player].ActiveTracker == nil) then
			QuestlogOptions[EQL3_Player].ActiveTracker = 0;
		end
		if(QuestlogOptions[EQL3_Player].ShowTrackerHeader == nil) then
			QuestlogOptions[EQL3_Player].ShowTrackerHeader = 0;
		end
...
function QuestLog_Update()
	...
	local questLogTitleText, level, questTag, sugGroup, isHeader, isCollapsed, isComplete, color;
...
--			questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete = GetQuestLogTitle(questIndex);
			questLogTitleText, level, questTag, sugGroup, isHeader, isCollapsed, isComplete = GetQuestLogTitle(questIndex);
			...
			if ( questTag ) then
				-- MODIFY GROUP TAGS TO INDICATE SUGGESTED GROUP SIZE
				if (questTag == "Group") then
					if (sugGroup > 0) then
						-- NEW QUESTS GET SUGGESTED GROUP SIZES
						questTag = questTag..": "..sugGroup;
					else
						-- OLD QUESTS GET OLD-SCHOOL "ELITE" TAG
						questTag = "Elite";
					end
				end
...
--		local questLogTitleText, level, questTag, isHeader, isCollapsed = GetQuestLogTitle(i);
		local questLogTitleText, level, questTag, _, isHeader, isCollapsed = GetQuestLogTitle(i);
...
--	local questLogTitleText, level, questTag, isHeader, isCollapsed = GetQuestLogTitle(questID);
	local questLogTitleText, level, questTag, _, isHeader, isCollapsed = GetQuestLogTitle(questID);
	if ( isHeader ) then
		if ( isCollapsed ) then
...
--		local questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete = GetQuestLogTitle(questIndex);
		local questLogTitleText, level, questTag, _, isHeader, isCollapsed, isComplete = GetQuestLogTitle(questIndex);
...
--	local questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete;
	local questLogTitleText, level, questTag, groupcnt, isHeader, isCollapsed, isComplete;
...
--		questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete =  old_GetQuestLogTitle(questIndex);
		questLogTitleText, level, questTag, groupcnt, isHeader, isCollapsed, isComplete =  old_GetQuestLogTitle(questIndex);
	end
...
--			questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete = old_GetQuestLogTitle(EQL3_Temp.savedQuestIDMap[questIndex].questID);
			questLogTitleText, level, questTag, groupcnt, isHeader, isCollapsed, isComplete = old_GetQuestLogTitle(EQL3_Temp.savedQuestIDMap[questIndex].questID);
		else
--			return EQL3_Temp.savedQuestIDMap[questIndex].header, 0, nil, true, nil, nil;
			return EQL3_Temp.savedQuestIDMap[questIndex].header, 0, nil, nil, true, nil, nil;
		end
	end
...
--	return questLogTitleText, level, questTag, isHeader, isCollapsed, isComplete;
	return questLogTitleText, level, questTag, groupcnt, isHeader, isCollapsed, isComplete;
end
--]]
...
--		questTitle, _, _, isHeader, _, isComplete = GetQuestLogTitle(questID);
		questTitle, _, _, _, isHeader, _, isComplete = GetQuestLogTitle(questID);
	...
	--clean up
	for quest in pairs(QuestlogOptions[EQL3_Player].CompletedQuests) do
