...
local VERSION = tonumber(("$Revision: 1749 $"):match("%d+"))

local WotLK = select(4, GetBuildInfo()) >= 30000
local PitBull = PitBull
local PitBull_PowerBar = PitBull:NewModule("PowerBar", "LibRockEvent-1.0", "LibRockTimer-1.0")
local self = PitBull_PowerBar
if PitBull.revision < VERSION then
	PitBull.version = "r" .. VERSION
	PitBull.revision = VERSION
	PitBull.date = ("$Date: 2008-10-13 20:31:52 +0000 (Mon, 13 Oct 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
local localization = (GetLocale() == "koKR") and {
	...
	["Runic Power Background"] = "룬 마력 배경",
...
function PitBull_PowerBar:OnEnable(first)
	-- bucketed events
	self:AddEventListener({UNIT_MANA=true, UNIT_RAGE=true, UNIT_FOCUS=true, UNIT_ENERGY=true, UNIT_RUNIC_POWER=true, UNIT_MAXMANA=true, UNIT_MAXRAGE=true, UNIT_MAXFOCUS=true, UNIT_MAXENERGY=true, UNIT_MAXRUNIC_POWER=true }, "UNIT_MANA", 0.05)
	self:AddEventListener("UNIT_DISPLAYPOWER", "UNIT_DISPLAYPOWER", 0.05)
	if WotLK then
		-- Timer to update the powerbar for the pet and player frames on a
		-- more regular basis.
		self:AddRepeatingTimer(0, "OnUpdate")
	end
end
...
function PitBull_PowerBar:UpdatePowerColor(unit, frame)
	...
	if colorByClass and UnitIsPlayer(unit) then
		...
		r,g,b = unpack(PitBull.colorConstants[class or "PRIEST"])
	...
	elseif powerType == 6 then
		if customColor then
			r,g,b = unpack(db.customColors.runicPower)
		else
			r,g,b = unpack(PitBull.colorConstants.runicPower)
		end
		if customBGColor then
			br,bg,bb = unpack(db.customColors.runicPowerBG)
		end
	else
...
function PitBull_PowerBar:OnUpdate()
		...
		args = {
			...
			customBGColors = {
				...
				args = {
					...
					runicPowerBG = WotLK and {
						passValue = group,
						passValue2 = "runicPowerBG",
						get = getColor,
						set = setColor,
						name = L["Runic Power Background"],
						desc = L["Runic Power Background"],
						type = 'color'
					} or nil,
				},
			},
