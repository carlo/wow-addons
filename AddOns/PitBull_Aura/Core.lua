...
local VERSION = tonumber(("$Revision: 1724 $"):match("%d+"))

local PitBull = PitBull
local PitBull_Aura = PitBull:NewModule("Aura", "LibRockTimer-1.0", "LibRockEvent-1.0")
local self = PitBull_Aura
PitBull:ProvideVersion("$Revision: 1724 $", "$Date: 2008-09-18 22:52:22 +0000 (Thu, 18 Sep 2008) $")

self.WoTLK = select(4, GetBuildInfo()) >= 30000
...
local localization = (GetLocale() == "koKR") and {
	...
	["Extra friend highlights"] = "별도 아군 강조",
	["Additional debuffs that will be highlighted on friends and yourself."] = "아군 및 자신에게 강조할 추가적인 디버프를 선택합니다.",
	["Add extra friend highlight"] = "별도 아군 디버프 추가",
	["Add a debuff to highlight on friends that is not listed."] = "목록에 없는 아군에게 강조할 디버프를 추가합니다.",
	...
	["Extra friend highlights"] = "额外的友方高亮",
	["Additional debuffs that will be highlighted on friends and yourself."] = "友好目标所要高亮的额外Debuff。",
	["Add extra friend highlight"] = "添加额外的友方高亮",
	["Add a debuff to highlight on friends that is not listed."] = "为友好目标所高亮的Debuff添加一个列表里没有的。",
	...
	["Extra friend highlights"] = "額外的友方目標高亮",
	["Additional debuffs that will be highlighted on friends and yourself."] = "選擇你想要對友方目標高亮的特定Debuff.",
	["Add extra friend highlight"] = "增加額外的友方目標高亮",
	["Add a debuff to highlight on friends that is not listed."] = "增加一個你想要對方目標高亮的Debuff, 該Debuff不在列表內.",
} or {}
...
function PitBull_Aura:OnUpdateAllFrames()
	...
	for unit, frame in PitBull:IterateWackyUnitFrames() do
		if lastUpdated[frame] < currentTime_1 then
			self:UpdateAuras(unit, frame)
		end
	end
