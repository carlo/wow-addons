...
function beql:InitAchievementTracker()
	...
	beql.ATrackerTitleText = beql.ATrackerTitle:CreateFontString("ATrackerTitleText","BACKGROUND","QuestWatchFontTemplate")
...
function beql:ATrackerEnable()
	...
	if not beql:IsEventRegistered("PLAYER_ENTERING_WORLD") and GetTrackedAchievement() then
...
function beql:TRACKED_ACHIEVEMENT_UPDATE()
	if GetTrackedAchievement() then
		beql.db.char.AchievementTracker.Achievement = GetTrackedAchievement()
	else
	...
	if not GetTrackedAchievement() then
		ATrackerBackdrop:Hide()
		return;
	else
		ATrackerBackdrop:Show()
...
function beql:ACHIEVEMENT_EARNED ()
	...
	if GetTrackedAchievement() == lastfinished then
