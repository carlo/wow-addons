...
local VERSION = tonumber(("$Revision: 1718 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-08-22 09:32:16 +0000 (Fri, 22 Aug 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
function PitBull_ComboPoints:OnEnable()
	...
	self:AddEventListener("UNIT_COMBO_POINTS")
end
...
function PitBull_ComboPoints:UNIT_COMBO_POINTS(ns, event, unit)
	if unit == "player" then
		return self:UpdateComboPoints()
	end
end
...
function PitBull_ComboPoints:UpdateComboPoints(frame)
	...
	if frame.comboPoints then
		local p = configMode and 5 or GetComboPoints("player")
		for i = 1, 5 do
			if p >= i then
