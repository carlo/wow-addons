...
local VERSION = tonumber(("$Revision: 1721 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-08-23 20:19:53 +0000 (Sat, 23 Aug 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
local localization = (GetLocale() == "koKR") and {
	...
	["Focus"] = "주시 대상",
	["Hides the standard focus frame."] = "기본 주시 대상 프레임을 숨깁니다.",
	["Cast bar"] = "시전바",
...
function PitBull_HideBlizzard:OnInitialize()
	...
	PitBull:RegisterGlobalSetting("HideBlizzard", function() return "@cache", {
		...
		args = {
			...
			focus = {
				name = L["Focus"],
				desc = L["Hides the standard focus frame."],
				passValue = "focus",
				disabled = not WoTLK,
			},
			castbar = {
...
function PitBull_HideBlizzard:HideBlizzardFocusFrame()
	if not WoTLK then
		return
	end
	if not focusFrameState then
		return
	end
...
function PitBull_HideBlizzard:ShowBlizzardFocusFrame()
	if not WoTLK then
		return
	end
	if focusFrameState then
		return
	end
