...
function QuartzPlayer:UNIT_SPELLCAST_DELAYED(unit)
	...
	self.delay = (self.delay or 0) + (startTime - oldStart)
end
...
function QuartzPlayer:UNIT_SPELLCAST_CHANNEL_UPDATE(unit)
	...
	self.delay = (self.delay or 0) + (oldStart - startTime)
end
do
	...
	Quartz.options.args.Player = {
		...
		args = {
			...
			copysettings = {
				...
				get = false,
				set = function(v)
					local from = self:AcquireDBNamespace(v)
					self:CopySettings(from.profile, db.profile)
					Quartz.ApplySettings()
				end,
				validate = {L["Target"], L["Focus"]},
				order = 504
			},
			header6 = {
				type = 'header',
