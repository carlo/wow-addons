...
function QuartzFocus:UNIT_SPELLCAST_DELAYED(unit)
	...
	delay = (delay or 0) + (startTime - oldStart)
end
...
function QuartzFocus:UNIT_SPELLCAST_CHANNEL_UPDATE(unit)
	...
	delay = (delay or 0) + (oldStart - startTime)
end
...
do
	...
	function QuartzFocus:ApplySettings()
		...
		elseif qpdb.border == L["None"] then
			backdrop.bgFile = "Interface\\Tooltips\\UI-Tooltip-Background"
			backdrop.tile = true
			backdrop.tileSize = 16
			backdrop.edgeFile = ''
			...
			copysettings = {
				...
				set = function(v)
					...
					if v == L["Player"] then
						...
						from = Quartz:AcquireDBNamespace(v)
					end
					Quartz:CopySettings(from.profile, db.profile)
					Quartz.ApplySettings()
				end,
				validate = {L["Player"], L["Target"]},
				order = 504
			},
		},
	}
