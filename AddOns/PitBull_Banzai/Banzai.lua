...
local VERSION = tonumber(("$Revision: 1690 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-07-08 18:16:42 +0000 (Tue, 08 Jul 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
function PitBull_Banzai:UpdateHealthbarColor(module, unit, frame)
	if not frame.healthBar or not UnitIsFriend("player",unit) then
