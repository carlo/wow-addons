...
local VERSION = tonumber(("$Revision: 1708 $"):match("%d+"))
local OMNICC_VERSION; if IsAddOnLoaded("OmniCC") then OMNICC_VERSION = tonumber((GetAddOnMetadata('OmniCC', 'Version'):gsub("%.",""))) end -- OmniCC version because OmniCC 2.1.0 changed the API
...
PitBull:ProvideVersion("$Revision: 1708 $", "$Date: 2008-08-08 01:20:28 +0000 (Fri, 08 Aug 2008) $")
local MetaLayout = PitBull.MetaLayout

local SharedMedia = Rock("LibSharedMedia-3.0")
-- registering a default sound
SharedMedia:Register('sound','Drop',"Sound\\interface\\DropOnGround.wav")
...
local localization = (GetLocale() == "koKR") and {
	...
	["Sound"] = "소리",
	["This plays a sound file when a totem expires or gets destroyed. See the Sounds-tab for more detailed options."] = "토템이 파괴되거나 완료시 소리를 재생합니다. 자세한 옵션은 소리-탭에서 볼 수 있습니다.",
	["Sounds"] = "소리",
	["Options relating to sound effects on totem events."] = "토템 이벤트의 소리 효과에 관련된 옵션입니다.",
	...
	["Suppress Cooldown Counts"] = "CooldownCount Addons unterdrücken",
	["Tries to suppress CooldownCount-like addons on the spiral timer. (Requires UI reload to change the setting!)"] = "Versucht CooldownCount-ähnliche Addons auf dem Spiraltimer zu unterdrücken. (UI muss neu geladen werden um aktiv zu werden!)",
	...
	["Sound"] = "Ton",
	["This plays a sound file when a totem expires or gets destroyed. See the Sounds-tab for more detailed options."] = "Spielt einen Ton ab sobald ein Totem ausläuft oder zerstört wird. Siehe den Toneffekte-Reiter für detailliertere Optionen.",
	["Sounds"] = "Toneffekte",
	["Options relating to sound effects on totem events."] = "Optionen für die Toneffekte bei Totem Ereignissen.",
...
local function getSoundForSlot(slot)
	if self.db.profile.deathsoundpaths and self.db.profile.deathsoundpaths[slot] then
		return self.db.profile.deathsoundpaths[slot]
	else
		return false
	end
end
local function getSoundPathForSlot(slot)
	local chosen = getSoundForSlot(slot)
	if SharedMedia and chosen then
		return SharedMedia:Fetch('sound', chosen)
	else
		return false
	end
end	
local function setSoundForSlot(slot, path)
	if self.db.profile.deathsoundpaths then
		self.db.profile.deathsoundpaths[slot] = path
...
function PitBull_TotemTimers:PLAYER_TOTEM_UPDATE(something, event, slot)
	...
	for frame in PitBull:IterateUnitFramesForUnit("player") do
		...
		elseif ( haveTotem ) then
			...
			-- Sound functions
			if getGenericOption('deathsound') and getSoundForSlot(slot) and not (event == nil) then
				--PitBull:Print(string.format('DEBUG: Playing Death sound for slot %s: %s', tostring(slot), tostring(getSoundPathForSlot(slot))))
				PlaySoundFile(getSoundPathForSlot(slot))
			end
		end
	end
...
function PitBull_TotemTimers:OnPopulateUnitFrame(unit, frame)
	...
	if not totemTimersFrame.texture then
		totemTimersFrame.texture = newFrame("Texture", totemTimersFrame, "BACKGROUND")
	end
	...
	for i=1, MAX_TOTEMS do
		...
		if not frame.border then
			frame.border = newFrame("Texture", frame, "OVERLAY")
		end
		...
		return {
			...
			args = {
				...
				grptotemsound = {
					...
					args = function() return getSoundOptionGroup() end,
					disabled = function() return not getGenericOption('deathsound') or getHide() end,
				},
			}
		}
	end
