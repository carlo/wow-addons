------------------------------------------------------------------------------
-- GFWOptions.lua
-- $Id: GFWOptions.lua 663 2008-10-13 08:08:59Z rick $
------------------------------------------------------------------------------
...
local LIBRARY_VERSION_MINOR = tonumber(string.match("$Revision: 663 $", "(%d+)") or 1)
...
local function optionsShow(panel)
	...
	for key, default in pairs(addonObj.defaults.profile) do
		local value = addonObj.db.profile[key];
		if (value == nil) then
		 	value = default;
		end
...
-- panel:CreateChildPanel(frameName, displayName)
local function optionsPanelCreateChildPanel(panel, frameName, displayName, description)
	local childPanel = CreateFrame("Frame", frameName);
	childPanel.parent = panel.name;
	childPanel.addon = panel.addon;
	childPanel.description = description;
	childPanel.name = displayName;
	childPanel.okay = optionsSave;
	childPanel.cancel = optionsCancel;
	childPanel.default = optionsReset;
	childPanel.refresh = optionsShow;
	
	-- inject API
	childPanel.CreateCheckButton = optionsPanelCreateCheckButton;
	childPanel.CreateSlider = optionsPanelCreateSlider;
	childPanel.CreateDropDown = optionsPanelCreateDropDown;
	
	childPanel:SetScript("OnShow", optionsShow);
...
function GFWOptions:CreateMainPanel(addonName, panelName, description)
	...
	panel.CreateChildPanel = optionsPanelCreateChildPanel;
