...
CharactersViewer.PaperDollFrame.SetStats = function()
	for i=1, NUM_STATS, 1 do
		...
		effectiveStat, posBuff, negBuff = CharactersViewer.Api.splitstats( CharactersViewer.Api.GetParam(CharactersViewer["CP"]["StatsLowerCase"][i]) )
		...
		if ( ( posBuff == 0 ) and ( negBuff == 0 ) ) then
			...
			if ( posBuff > 0 or negBuff < 0 ) then
				tooltipText = tooltipText.." ("..(effectiveStat)..FONT_COLOR_CODE_CLOSE;
			end
			if ( posBuff > 0 ) then
			...
			if ( negBuff < 0 ) then
				text:SetText(RED_FONT_COLOR_CODE..(effectiveStat + posBuff - negBuff)..FONT_COLOR_CODE_CLOSE);
			else
				text:SetText(GREEN_FONT_COLOR_CODE..(effectiveStat + posBuff - negBuff)..FONT_COLOR_CODE_CLOSE);
			end
		end
	end
end;
...
CharactersViewer.PaperDollFrame.SetArmor = function(unit, prefix)
	...
	CVPaperDollFormatStat(ARMOR, base + effectiveArmor, posBuff, negBuff, frame, text);
	local playerLevel = UnitLevel(unit);
	local armorReduction = (base + effectiveArmor)/((85 * playerLevel) + 400);
