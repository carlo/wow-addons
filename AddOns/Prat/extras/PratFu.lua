...
if not PRAT_LIBRARY:HasInstance(PRATLIB.BARMENU) then return end

local L = PRAT_LIBRARY(PRATLIB.LOCALIZATION):new("PratFu")
...
PratFu = PRAT_LIBRARY(PRATLIB.BASE):new(PRATLIB.DEBUG, PRATLIB.DATASTORE, PRATLIB.EVENTS, PRATLIB.BARMENU)
PratFu:RegisterDB("PratFuDB")
...
local mm = Prat:NewModule(Prat:RequestModuleName("minimap"))
...
local args = PRAT_LIBRARY(PRATLIB.BARMENU):GetAceOptionsDataTable(PratFu)
...
function PratFu:OnEnable()
	self:RegisterEvent("Prat_DebugModeChanged", "Update")
...
local dewdrop = PRAT_LIBRARY(PRATLIB.DROPDOWNMENU)
...
function PratFu:OnClick(button)
	if button == 'LeftButton' then
		...
		elseif self.waterfallavailable then
