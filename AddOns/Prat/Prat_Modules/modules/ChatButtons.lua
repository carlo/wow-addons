...
L:RegisterTranslations("esES", function() return {
    ...
    ["Toggles chat%d arrows on and off."] = "Activa y desactiva los botones de la ventana de %d ",
...
-- create prat module
Prat_ChatButtons = Prat:NewModule(PRAT_MODULE)
Prat_ChatButtons.revision = tonumber(string.sub("$Revision: 55577 $", 12, -3))
...
Prat_ChatButtons.Categories = { cat.BUTTON, cat.FRAME }
...
function Prat_ChatButtons:OnModuleOnUpdateHook(this, elapsed)
    ...
    self:ChatFrame_OnUpdateTextFlow(elapsed)
...
function Prat_ChatButtons:ChatFrame_OnUpdateTextFlow(elapsed)
    if self.db.profile.position == "RIGHTINSIDE" then
        local id = this:GetID()
