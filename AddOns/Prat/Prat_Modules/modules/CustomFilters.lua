...
Prat_CustomFilters = Prat:NewModule(PRAT_MODULE, "Sink-1.0")
Prat_CustomFilters.revision = tonumber(string.sub("$Revision: 52090 $", 12, -3))
...
Prat_CustomFilters.Categories	= { cat.FILTER, cat.FORMATTING, cat.TEXT, cat.COLORS }
...
function Prat_CustomFilters:GetModuleOptions()
    media = PRAT_LIBRARY(PRATLIB.MEDIA)
    ...
    self.moduleOptions.args.output =  PRAT_LIBRARY(PRATLIB.NOTIFICATIONS):GetAceOptionsDataTable(self).output 
...
function Prat_CustomFilters:OnModuleEnable(first)
    deformat = PRAT_LIBRARY(PRATLIB.PARSING)
    ...
    self:RegisterEvent(Prat.Events.POST_ADDMESSAGE)
end
...
function Prat_CustomFilters:Prat_PostAddMessage(message, frame, event, text, r, g, b, id)
    ...
    self:Debug("Prat_PostAddMessage", text, message.CF_SINK, message.CF_SOUND)

	if message.CF_SINK then	
