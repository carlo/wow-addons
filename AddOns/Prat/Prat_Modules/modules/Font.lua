...
L:RegisterTranslations("esES", function() return {
    ...
    ["autorestore_name"] = "Recuperaci\195\179n Autom\195\161tica",
...
-- create prat module
Prat_Font = Prat:NewModule(PRAT_MODULE)
Prat_Font.revision = tonumber(string.sub("$Revision: 51543 $", 12, -3))
...
Prat_Font.Categories = { cat.TEXT, cat.FORMATTING }
...
function Prat_Font:OnModuleEnable()
    ...
    media = media or PRAT_LIBRARY(PRATLIB.MEDIA)
    self.oldsize = {}
    self:BuildFontList()
...
function Prat_Font:ConfigureAllChatFrames()
    ...
    for i = 1,NUM_CHAT_WINDOWS do
        self:SetFontSize(i, self.db.profile.size[i])
    end
    -- apply font flag settings
    if not self.db.profile.monochrome then
        self:SetFontMode(self.db.profile.outlinemode)
    else
        self:SetFontMode(self.db.profile.outlinemode..", MONOCHROME")
    end
end
...
function Prat_Font:SetShadowClr(r,g,b)
    self.db.profile.shadowcolor = self.db.profile.shadowcolor or {}
    local h = self.db.profile.shadowcolor
    h.r, h.g, h.b = r, g, b
    self:ConfigureAllChatFrames()
end
