...
Prat_PlayerNames.revision = tonumber(string.sub("$Revision: 55577 $", 12, -3))
...
Prat_PlayerNames.Categories	= { cat.NAME, cat.FORMATTING, cat.TEXT, cat.INFO, cat.LINK }	-- eg { cat.TEXT, cat.INFO }
...
local Glory = PRAT_LIBRARY:HasInstance(PRATLIB.PVP, false) and PRAT_LIBRARY(PRATLIB.PVP)


function Prat_PlayerNames:OnModuleEnable()
    self:Debug("OnModuleEnable")

	self:RegisterEvent("Prat_FrameMessage")
...
function Prat_PlayerNames:AceLibrary_Register(major)
    if major == PRATLIB.PVP then
        Glory = PRAT_LIBRARY(PRATLIB.PVP)
    end
end
...
function Prat_PlayerNames:updateAll()
	...
	if self.db.profile.usewho then 
        ...
        who = who or PRAT_LIBRARY:HasInstance(PRATLIB.WHO) and PRAT_LIBRARY(PRATLIB.WHO)
...
function Prat_PlayerNames:GetClassColor(class)
    if class == "Shaman" then
        return Prat.Colors.SHAMAN or BC:GetHexColor(class)
    else
...
function Prat_PlayerNames:TabComplete(enabled)
    if enabled then
        if not PRAT_LIBRARY(PRATLIB.TABCOMPLETION):IsTabCompletionRegistered("Prat_PlayerNames") then
            PRAT_LIBRARY(PRATLIB.TABCOMPLETION):RegisterTabCompletion("Prat_PlayerNames", "",
                function(t, text, pos)
        ...
        if PRAT_LIBRARY(PRATLIB.TABCOMPLETION):IsTabCompletionRegistered("Prat_PlayerNames") then
            PRAT_LIBRARY(PRATLIB.TABCOMPLETION):UnregisterTabCompletion("Prat_PlayerNames")
        end
    end
end
