...
-- create prat module
Prat_Scroll = Prat:NewModule(PRAT_MODULE)
Prat_Scroll.revision = tonumber(string.sub("$Revision: 51543 $", 12, -3))
...
Prat_Scroll.Categories	= { cat.FRAME, cat.BEHAVIOR }	-- eg { cat.TEXT, cat.INFO }
    ...
    self.moduleOptions = {
        ...
        args = {
            ...
            lowdowndelay = {
                ...
                type = "range",
                order = 220,
                get = function() return self.db.profile.lowdowndelay end,
                set = function(v) self.db.profile.lowdowndelay = v end,
                min = 1,
                max = 60,
                step = 1,
            },
...
function Prat_Scroll:OnModuleEnable()
    ...
    for i = 1,NUM_CHAT_WINDOWS do
        self:MouseWheel(i,self.db.profile.MouseWheel[i])
       	self:LowDown(i,self.db.profile.LowDown[i])
    end
end
...
function Prat_Scroll:LowDown(id, enabled)
    self:Debug(id, enabled)

	local name = "ChatFrame"..id
    local cf = getglobal(name)
    ...
    if enabled then
		for _,func in ipairs(funcs) do
			local f = function(cf)
				...
				self:ScheduleEvent(name.."DownTimeout", self.ResetFrame, self.db.profile.lowdowndelay, self, name)
			end
...
function Prat_Scroll:ResetFrame(name)
	local cf = getglobal(name)
