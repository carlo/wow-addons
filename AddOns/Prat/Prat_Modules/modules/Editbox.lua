...
local SharedMedia = PRAT_LIBRARY(PRATLIB.MEDIA)
...
L:RegisterTranslations("esES", function() return {
    ...
    ["Set Width (Nb: Only Enabled If The Editbox Is Undocked)"] = "Anchura",
...
-- create prat module
Prat_Editbox = Prat:NewModule(PRAT_MODULE)
Prat_Editbox.revision = tonumber(string.sub("$Revision: 53180 $", 12, -3))
...
function Prat_Editbox:GetModuleOptions()
    self.moduleOptions = {
		...
		max = 30,
		step = 1,
		desc = "Set the width of the edit box's border",
		set = function(width)
		...
		disabled = function() return not self.db.profile.hideborder end,
	    },
	    bordercolour = {
		name = 'Border colour',
		order = 220,
		desc = 'Set the edit box border colour',
		type = 'color',
		set = function(r, g, b, a)
		...
		max = 30,
		step = 1,
		desc = "Set the amount of padding inside the edit box",
...
function Prat_Editbox:GetBackdropTexture()
    return SharedMedia:Fetch('statusbar', self.db.profile.texture)
end

function Prat_Editbox:SetBackdrop()
