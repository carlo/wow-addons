...
Prat_ChannelNames.revision = tonumber(string.sub("$Revision: 53322 $", 12, -3))
...
Prat_ChannelNames.Categories = { cat.CHANNEL, cat.FORMATTING, cat.TEXT }
...
--[[ 
    ...
    for _, v in ipairs(orderMap) do
        args[name].text
        args[name].desc = string.format(L["%s settings."], text)

        local set = args[name].args.set
        set.name = text
        set.desc = string.format(L["Use a custom replacement for the chat %s text."], text)
    end
    for i=1,10 do
        args[name].name = string.format(text, num),
        args[name].desc = string.format(L["%s settings."], text)

        local set = args[name].args.set
        set.name = string.format(text, num),
        set.desc = string.format(L["Use a custom replacement for the chat %s text."], string.format(L["Channel %d"], num)),
    end
--]]
end
...
function Prat_ChannelNames:MakeChan_Link(message)
    ...
    if prof.chanlink and cnum then
		message.nN = "|H"
