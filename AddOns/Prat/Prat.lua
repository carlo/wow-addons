...
local L = PRAT_LIBRARY(LIB.LOCALIZATION):new("Prat")
...
Prat = PRAT_LIBRARY(LIB.BASE):new(unpack(Mixins))

Prat.revision = tonumber(string.sub("$Revision: 53322 $", 12, -3))
Prat:RegisterDB("PratDB", "PratPerCharDB")
...
-- Status flags.
Prat.Statuses = {
    CONFIRMEDTAINT = "CONFIRMEDTAINT", -- Module taints/or causes the client not to function
    TAINT = "TAINT",                    -- Module taints(maybe) - no harmful effect observed
    BROKEN = "BROKEN",                 -- This module is not fully functional
    BADINIT = "BADINIT",               -- This module did not init properly
    BADENABLE = "BADENABLE",           -- This module did not enable properly
    PRATEVENTS = "PRATEVENTS",          -- This module uses Prat events to process chat text
    NEEDSCATEGORIES = "NEEDCATS",       -- This says we still need some categories on this module
}
...
local builtinSounds = {
    ["popup"] = "Interface\\AddOns\\Prat\\sounds\\chatmod1.mp3",
    ["Bell"] = "Interface\\AddOns\\Prat\\sounds\\chatmod3.mp3",
	["Heart"] = "Interface\\AddOns\\Prat\\Sounds\\Heart.wav",
	["Kachink"] = "Interface\\AddOns\\Prat\\Sounds\\Kachink.wav",
	["Link"] = "Interface\\AddOns\\Prat\\Sounds\\Link.wav",
...
--[[
        optsep90 = {
            order = 90,
            type = 'header',
            hidden = function() return #Prat.Updated == 0 end
        },

        updatedmods = {
            order = 95,
            type = "group",
            name = L["Updated Modules"],
            desc = L["Updated Modules"],
            hidden = function()
                        if  #Prat.Updated == 0 then return true end

                         local a = Prat.Options.args.updatedmods.args
                         for i,v in ipairs(Prat.Updated) do
                            a["upt"..i] = v.moduleOptions
                         end
                    end,
            args = {},
         },
...
--
Prat:RegisterChatCommand({"/prat"}, Prat.ConsoleOptions)

function Prat:OnInitialize()
    Prat:RegisterDefaults("profile", {
        debug = false,
        dumpSplit = false,
        dumpEventInfo = false,
   ...
   for k,v in pairs(o) do
    ...
    self:RegisterEvent(Prat.Events.MODULE_ENABLED)
    self:RegisterEvent(Prat.Events.MODULE_DISABLED)
    ...
    PRAT_LIBRARY(LIB.CONSOLEMENU):InjectAceOptionsTable(Prat, Prat.Options)
...
function Prat:PostEnable()
    ...
    self:TriggerEvent(Prat.Events.SECTIONS_UPDATED)
...
-- Hack from blizzard's code
    ...
    if cmd and IsSecureCmd(cmd) then
        DBG_PATTERN("The command '"..cmd.."' is secure.")
		return
	end
...
function Prat:SetItemRef(link, text, button, ...)
    DUMP_LINK("SetItemRef", link, text, button)

    local old_SCF = SELECTED_CHAT_FRAME
    SELECTED_CHAT_FRAME = this
    
    for i,reg_link in ipairs(self.LinkRegistry) do
        if reg_link.linkid == strsub(link, 1, strlen(reg_link.linkid)) then
            if (reg_link.linkfunc(reg_link.handler, link, text, button, ...) == false) then
                SELECTED_CHAT_FRAME = old_SCF
                return false
            end
        end
    end

    self:ScheduleEvent("Prat_PostSetItemRef", function() SELECTED_CHAT_FRAME = old_SCF end , 0.1)
...
function Prat:ChatFrame_MessageEventHandler(event)
    ...
    -- This is here temporarily so we dont process the combatlog
--    if strsub(event, 1, 4) == "CHAT" then
...
--    end
    ...
    if not info then
       return self.hooks["ChatFrame_MessageEventHandler"](event)
        ...
        self:TriggerEvent(Prat.Events.FRAME_MESSAGE, message, this, event)
        ...
        if type(m.OUTPUT) == "string" and not m.DONOTPROCESS then
            ...
            self:TriggerEvent(Prat.Events.PRE_ADDMESSAGE, message, this, event, self:BuildChatText(message), r,g,b,id )
            ...
            if not m.DONOTPROCESS then 
                ...
                -- We have called addmessage by now
    
                DUMP_OUTPUT("Prat_PostAddMessage", m)
    
                self:TriggerEvent(Prat.Events.POST_ADDMESSAGE,  m, this, event, m.OUTPUT, r,g,b,id)
            end
        end
...
function Prat:PluginRenderAsChatString(frame, message, event)
    ...
    self:TriggerEvent(Prat.Events.FRAME_MESSAGE, message, frame, event)
    ...
    if (type(message.OUTPUT) == "string" or process) and not message.DONOTPROCESS then
        ...
        self:TriggerEvent(Prat.Events.PRE_ADDMESSAGE, message, frame, event, self:BuildChatText(message), r,g,b,id)

        -- Pattern Matches Put Back IN
        message.MESSAGE = self:ReplaceMatches(message.MESSAGE, "FRAME")

        if not message.DONOTPROCESS then         
            ...
            self:TriggerEvent(Prat.Events.POST_ADDMESSAGE,  message, frame, event, message.OUTPUT, r,g,b,id)
        end
...
function Prat:RegisterModule(name, module)
    local modkey = module.name
    local ps = Prat.Statuses

    DBG_MODULE("RegisterModule() - module="..modkey..", name="..name..", command="..(module.consoleCmd or name))
    ...
    if name_owner ~= module.name then
        DBG_MODULE("RegisterModule() NAME ERROR - name: "..name.."   owner: "..(name_owner or "nil"))
        module.status = "BROKEN"
    else
        if self.Registered[modkey] then
            -- Oh No, Registering again is a no-no!!
            DBG_MODULE("RegisterModule() DUPLICATE REGISTRATION ERROR - name: "..name)
            modules.status = "BROKEN"
        else
    ...
    -- TODO: store this by what?
 --   if self.db.profile.mod_versions[modkey] < revision then
--        table.insert(self.Updated, module)
--    end

    if module.db then
        DBG_MODULE("About to do something bad for - "..module.name)
        module:RegisterDefaults("profile", module.defaultDB or {})
    else
        module.db = self:AcquireDBNamespace(modkey)
        self:RegisterDefaults(modkey, "profile", module.defaultDB or {})
    end
end
...
function Prat:RegisterModuleOptions(name, module)
    ...
    local mL = PRAT_LIBRARY(LIB.LOCALIZATION):new(modkey)
    ...
    --local name = mL:HasTranslation("module_name") and L["module_name"]
    --local desc = mL:HasTranslation("module_desc") and L["module_desc"]
    opts.name = opts.name or module.moduleName or name or module.name
--    if module.consoleName then
--        opts.guiName = opts.guiName or module.guiName or opts.name
--        opts.name = nil
--        opts.cmdName = module.consoleName
---    end
...
Prat.modulePrototype.Categories = nil
...
function Prat.modulePrototype:OnInitialize()
    ...
    -- Register the module
    self.core:RegisterModule(self.moduleName or self.name, self)
...
function Prat.modulePrototype:OnEnable(first)
    ...
    if first then 
        self.core:RegisterModuleOptions(self.name, self)
        self:OnModuleInit()
    end
    ...
    if not self.db.profile.on then
        DBG_MODULE("Module ["..self.name.."] should not be active")
        self.core:ToggleModuleActive(self, false)
        return
    end
