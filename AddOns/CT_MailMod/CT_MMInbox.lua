	...
	if ( this.num and this.elapsed ) then
		...
		if ( this.elapsed <= 0 ) then
			...
			if ( this.id[1] ) then
				...
				if ( success == 1 ) then
					for key, va in pairs(this.id) do
						if ( va > val ) then
							this.id[key] = va-1;
						end
					end
				...
				if ( this.num == 0 ) then
					...
					if ( not GetInboxHeaderInfo(1) ) then
...
function CT_MMInbox_SetSelected()
	...
	if ( not this:GetChecked() ) then
		for k, v in pairs(CT_MMInbox_SelectedItems) do
			if ( v == id ) then
...
function CT_MMForward_ScanItems()
	local old = { };
	for k, v in pairs(CT_MMForward_BagLinks) do
...
local oldSMMfunc = SendMailMoney.onvalueChangedFunc;
SendMailMoney.onvalueChangedFunc = function()
	if ( oldSMMfunc ) then
		oldSMMfunc();
	end
	local subject = SendMailSubjectEditBox:GetText();
	if ( subject == "" or string.find(subject, "^%[%d+G, %d+S, %d+C%]$") ) then
		local copper, silver, gold = SendMailMoneyCopper:GetText(), SendMailMoneySilver:GetText(), SendMailMoneyGold:GetText();
