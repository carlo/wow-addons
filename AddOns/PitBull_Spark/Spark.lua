...
local VERSION = tonumber(("$Revision: 1750 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-10-13 20:51:10 +0000 (Mon, 13 Oct 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
local LDM
if class == "DRUID" and not WoTLK then
  LDM = Rock("LibDruidMana-1.0", false, true)
end
...
function PitBull_Spark:DoRegistrations()
	if not WoTLK and (class == "ROGUE" or class == "DRUID") then
		current_energy = 0
...
if not WoTLK and (class == "ROGUE" or class == "DRUID") then --energy-users functions
