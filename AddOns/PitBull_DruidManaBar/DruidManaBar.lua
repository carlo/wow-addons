...
local VERSION = tonumber(("$Revision: 1747 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-10-13 00:44:57 +0000 (Mon, 13 Oct 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
function PitBull_DruidManaBar:UNIT_MANA(ns, event, arg1)
	...
	self:Update(nil, UnitPower("player",0), UnitPowerMax("player",0))
end
...
function PitBull_DruidManaBar:OnUpdateFrame(unit, frame)
	if frame.druidManaBar then
		if not WoTLK then
			...
			self:Update(frame, UnitPower("player", 0), UnitPowerMax("player", 0))
		end
		self:UpdateColor(frame)
	end
end
