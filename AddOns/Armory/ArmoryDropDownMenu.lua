--[[
    ...
    Revision: $Id: ArmoryDropDownMenu.lua,v 1.2, 2008-11-11 10:16:25Z, Maxim Baars$
...
function ArmoryDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList)
	...
	for i = 1, ARMORY_DROPDOWNMENU_MAXLEVELS, 1 do
		dropDownList = getglobal("ArmoryDropDownList"..i);
		if ( i >= ARMORY_DROPDOWNMENU_MENU_LEVEL or frame:GetName() ~= ARMORY_DROPDOWNMENU_OPEN_MENU ) then
			...
			for j=1, ARMORY_DROPDOWNMENU_MAXBUTTONS, 1 do
				button = getglobal("ArmoryDropDownList"..i.."Button"..j);
				button:Hide();
			end
			dropDownList:Hide();
		end
...
function ArmoryDropDownMenu_CreateFrames(level, index)
	...
	while ( index > ARMORY_DROPDOWNMENU_MAXBUTTONS ) do
		...
		for i=1, ARMORY_DROPDOWNMENU_MAXLEVELS do
			local newButton = CreateFrame("Button", "ArmoryDropDownList"..i.."Button"..ARMORY_DROPDOWNMENU_MAXBUTTONS, getglobal("ArmoryDropDownList"..i), "ArmoryDropDownMenuButtonTemplate");
...
function ArmoryDropDownMenu_AddButton(info, level)
	...
	local listFrame = getglobal("ArmoryDropDownList"..level);
	...
	local frame = getglobal(ARMORY_DROPDOWNMENU_OPEN_MENU);
	...
	if ( not frame ) then
		frame = getglobal(ARMORY_DROPDOWNMENU_INIT_MENU);
	end
	...
	if ( info.hasColorSwatch ) then
		getglobal("ArmoryDropDownList"..level.."Button"..index.."ColorSwatch".."NormalTexture"):SetVertexColor(info.r, info.g, info.b);
		button.r = info.r;
		button.g = info.g;
		button.b = info.b;
		colorSwatch:Show();
...
function ArmoryDropDownMenu_Refresh(frame, useValue, dropdownLevel)
	...
	for i=1, ARMORY_DROPDOWNMENU_MAXBUTTONS do
		button = getglobal("ArmoryDropDownList"..dropdownLevel.."Button"..i);
		checked = nil;
		...
		checkImage = getglobal("ArmoryDropDownList"..dropdownLevel.."Button"..i.."Check");
		if ( checked ) then
			if ( useValue ) then
	...
	getglobal("ArmoryDropDownList"..dropdownLevel):SetWidth(maxWidth+15);
end
...
function ArmoryDropDownMenu_GetSelectedID(frame)
	if ( frame.selectedID ) then
		...
		local button;
		for i=1, ARMORY_DROPDOWNMENU_MAXBUTTONS do
			button = getglobal("ArmoryDropDownList"..ARMORY_DROPDOWNMENU_MENU_LEVEL.."Button"..i);
			-- See if checked or not
			if ( ArmoryDropDownMenu_GetSelectedName(frame) ) then
				if ( button:GetText() == ArmoryDropDownMenu_GetSelectedName(frame) ) then
					return i;
...
function ArmoryToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button)
	...
	if ( listFrame:IsShown() and (ARMORY_DROPDOWNMENU_OPEN_MENU == tempFrame:GetName()) ) then
		...
		if ( level == 1 ) then
			...
			if ( not dropDownFrame ) then
				dropDownFrame = getglobal(ARMORY_DROPDOWNMENU_OPEN_MENU);
			end
			listFrame:ClearAllPoints();
...
function ArmoryCloseDropDownMenus(level)
	...
	for i=level, ARMORY_DROPDOWNMENU_MAXLEVELS do
		getglobal("ArmoryDropDownList"..i):Hide();
	end
end
...
function ArmoryDropDownMenu_ClearAll(frame)
	...
	for i=1, ARMORY_DROPDOWNMENU_MAXBUTTONS do
		...
		checkImage = getglobal("ArmoryDropDownList"..ARMORY_DROPDOWNMENU_MENU_LEVEL.."Button"..i.."Check");
		checkImage:Hide();
	end
end
...
function ArmoryDropDownMenu_GetCurrentDropDown()
	if ( ARMORY_DROPDOWNMENU_OPEN_MENU ) then
		return getglobal(ARMORY_DROPDOWNMENU_OPEN_MENU);
	elseif ( ARMORY_DROPDOWNMENU_INIT_MENU ) then
		return getglobal(ARMORY_DROPDOWNMENU_INIT_MENU);
	end
...
function ArmoryDropDownMenu_DisableButton(level, id)
	getglobal("ArmoryDropDownList"..level.."Button"..id):Disable();
end

function ArmoryDropDownMenu_EnableButton(level, id)
	getglobal("ArmoryDropDownList"..level.."Button"..id):Enable();
end

function ArmoryDropDownMenu_SetButtonText(level, id, text, colorCode)
	local button = getglobal("ArmoryDropDownList"..level.."Button"..id);
	if ( colorCode) then
...
function ArmoryDropDownMenu_GetValue(id)
	...
	local button = getglobal("ArmoryDropDownList1Button"..id);
	if ( button ) then
		return getglobal("ArmoryDropDownList1Button"..id).value;
	else
		return nil;
	end
end
