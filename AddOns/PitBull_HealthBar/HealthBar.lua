...
local VERSION = tonumber(("$Revision: 1751 $"):match("%d+"))
...
if PitBull.revision < VERSION then
	...
	PitBull.date = ("$Date: 2008-10-13 23:54:23 +0000 (Mon, 13 Oct 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
function PitBull_HealthBar:OnEnable(first)
	...
	self:AddEventListener("UNIT_FACTION", "UNIT_FACTION", 0.05)
        if WotLK then
                -- Timer to update the healthbar for the pet and player frames on a
                -- more regular basis.
...
function PitBull_HealthBar:OnUpdate()
