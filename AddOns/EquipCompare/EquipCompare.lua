...
-- occupying slotid
local function ShowComparisonTooltip(parent, slotid, link)
	...
	if ( EquipCompare_CharactersViewer and CharactersViewer.index and
		...
		ComparisonTooltip1:SetHyperlinkCompareItem(link, 1);
	end
...
function EquipCompare_ShowCompare(tooltip)
	...
	local ttext, itype, slotid, other, link, leftAlign;
	local i, cvplayer;
	...
	if ( slotid and link ) then
		...
		-- Display a comparison tooltip and set its contents to currently equipped item
		leftAlign = ShowComparisonTooltip(tooltip, slotid, link);
		...
		if ( other ) then
			if ( ComparisonTooltip1:IsVisible() ) then
				...
				if ( EquipCompare_CharactersViewer and CharactersViewer.index and
					...
					ComparisonTooltip2:SetHyperlinkCompareItem(link, 2);
				end
				...
				ShowComparisonTooltip(tooltip, other, link);
			end
		end
