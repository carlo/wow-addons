...
local function TriggersTab_SaveTriggerSettings(settings, triggerKey)
 ...
 MSBTProfiles.SetOption("triggers." .. triggerKey, "mainConditions", settings.mainConditions);
 MSBTProfiles.SetOption("triggers." .. triggerKey, "secondaryConditions", settings.secondaryConditions);
 MSBTProfiles.SetOption("triggers." .. triggerKey, "triggerEvents", settings.triggerEvents);
 MSBTTriggers.UpdateTriggers();
end
...
local function TriggersTab_Create()
 ...
 button:SetClickHandler(
   function (this)
    ...
    configTable.parentFrame = tabFrames.triggers;
    configTable.anchorFrame = this;
    --configTable.validateHandler = TriggersTab_ValidateScrollAreaName;
...
local function CooldownsTab_Create()
 ...
 -- Cooldown icons button.
 checkbox = MSBTControls.CreateCheckbox(tabFrame);
 local objLocale = MSBTLocale.CHECKBOXES["cooldownIcons"];
 checkbox:Configure(24, objLocale.label, objLocale.tooltip);
 checkbox:SetPoint("TOPLEFT", controls.cooldownSlider, "BOTTOMLEFT", 0, -20);
 checkbox:SetClickHandler(
   function (this, isChecked)
    MSBTProfiles.SetOption(nil, "cooldownIconsDisabled", not isChecked);
   end
 );
 controls.cooldownIconsCheckbox = checkbox;

 -- Cooldown exclusions button.
 button = MSBTControls.CreateOptionButton(tabFrame);
 objLocale = MSBTLocale.BUTTONS["cooldownExclusions"];
 button:Configure(20, objLocale.label, objLocale.tooltip);
 button:SetPoint("TOPLEFT", controls.cooldownIconsCheckbox, "BOTTOMLEFT", 0, -40);
 button:SetClickHandler(
   function (this)
...
local function CooldownsTab_OnShow()
 ...
 controls.cooldownIconsCheckbox:SetChecked(not currentProfile.cooldownIconsDisabled);
end
...
local function SkillIconsTab_OnShow()
 ...
 -- Disable controls if the icon support module isn't loaded.
 if (not MSBTIconSupport) then DisableControls(controls); end
end
